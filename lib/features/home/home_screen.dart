...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _setBookingStatus(
    ...
    try {
      await _bookingRepository.setBookingStatus(
  ...
  Future<void> _filterWorkers(String query) async {
    ...
    for (var doc in summarySnap.docs) {
      final data = doc.data();
      final avg = double.tryParse(data['avgRating']?.toString() ?? '') ?? 0.0;
  ...
  Future<void> _logout() async {
    try {
      await _authRepository.clearActiveSession();
      if (!mounted) return;
  ...
  Widget _buildWorkerDashboard() {
    return FutureBuilder<List<Map<String, dynamic>>>(
      future: _bookingRepository.loadBookings(widget.name),
        ...
        children: [
          ...
          Expanded(
            child: StreamBuilder<QuerySnapshot>(
              stream: FirebaseFirestore.instance.collection('summaries').snapshots(),
              builder: (context, snapshot) {
