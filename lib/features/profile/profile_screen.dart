...
import '../feedback/worker_feedback_screen.dart';
...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Widget _workerBookingsSection(String workerName) {
    return FutureBuilder<List<Map<String, dynamic>>>(
      future: _bookingRepository.loadBookings(workerName),
  ...
  Widget _workerSummarySection(String workerName) {
    return FutureBuilder<DocumentSnapshot>(
      future: FirebaseFirestore.instance.collection('summaries').doc(workerName).get(),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
        ...
        if (!snapshot.hasData || !snapshot.data!.exists) {
          return const Text('No feedback summary yet.', style: TextStyle(color: Colors.grey));
        }
        ...
        return GestureDetector(
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (_) => WorkerFeedbackScreen(workerName: workerName),
              ),
            );
          },
          child: Container(
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          if (_isMyProfile && widget.isWorker) _workerBookingsSection(widget.name),
          const SizedBox(height: 20),
